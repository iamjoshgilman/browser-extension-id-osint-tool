...
interface UseBulkSearchReturn {
  ...
  loading: boolean
  error: string | null
  search: (extensionIds: string[], stores: string[]) => Promise<void>
  clearResults: () => void
}
...
export function useBulkSearch(): UseBulkSearchReturn {
  ...
  const search = useCallback(async (extensionIds: string[], stores: string[]) => {
    setLoading(true)
    setError(null)
    setTotalSearched(extensionIds.length)
    try {
      const response = await bulkSearchExtensions({ extension_ids: extensionIds, stores })
