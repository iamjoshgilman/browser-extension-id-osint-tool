...
interface UseSearchReturn {
  ...
  loading: boolean
  error: string | null
  search: (extensionId: string, stores: string[], includePermissions?: boolean) => Promise<void>
  clearResults: () => void
}
...
export function useSearch(): UseSearchReturn {
  ...
  const search = useCallback(
    async (extensionId: string, stores: string[], includePermissions = false) => {
      setLoading(true)
      setError(null)
      try {
        const response = await searchExtension({
